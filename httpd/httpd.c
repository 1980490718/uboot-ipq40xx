...
#ifdef CONFIG_WEB_BACKUP_FIRMWARE
...
#ifdef CONFIG_WEB_MACRW
int web_macrw_handle(int argc, char **argv, char *resp_buf, int bufsize);
#endif
#ifdef CONFIG_WEB_BACKUP_FIRMWARE
int web_backupfw_handle(int argc, char **argv, char *resp_buf, int bufsize);
#endif
#ifdef CONFIG_WEB_SETENV
...
#if defined(CONFIG_WEB_MACRW) || defined(CONFIG_WEB_BACKUP_FIRMWARE) || defined(CONFIG_WEB_SETENV)
// url decode
...
#if defined(CONFIG_WEB_MACRW) || defined(CONFIG_WEB_SETENV)
	hs->is_macrw_resp = 0;
	hs->macrw_len = 0;
#endif
#ifdef CONFIG_WEB_BACKUP_FIRMWARE
...
#if defined(CONFIG_WEB_MACRW) || defined(CONFIG_WEB_SETENV)
// POST 请求处理函数 (仅用于 MACRW 和 SETENV)
...
#endif
#ifdef CONFIG_WEB_BACKUP_FIRMWARE
				if (!handled && strncmp((char *)&uip_appdata[4], "/download_firmware", 18) == 0) {
...
#endif
#ifdef CONFIG_WEB_SETENV
				if (!handled && strncmp((char *)uip_appdata, "POST /setenv", 12) == 0) {
					char resp_buf[8192];
...
#endif
				if (!handled && strncmp((char *)uip_appdata, "POST /reset", 11) == 0) {
...
#ifdef CONFIG_WEB_BACKUP_FIRMWARE
				if (!handled && strncmp((char *)uip_appdata, "POST /read_firmware", 19) == 0) {
...
#endif // defined(CONFIG_WEB_MACRW) || defined(CONFIG_WEB_BACKUP_FIRMWARE) || defined(CONFIG_WEB_SETENV)
			if (hs->state == STATE_FILE_REQUEST) {
				for (i = 4; i < 30; i++) {
