...
#else
int web_handle_read(char *response_buffer, size_t buffer_size) {
	snprintf(response_buffer, buffer_size, "Error: Web backup feature is disabled in this build\n");
	return -1;
}

int web_handle_download(unsigned char **firmware_data, unsigned int *firmware_size) {
	printf("Error: Web backup feature is disabled in this build\n");
	return -1;
}
#endif
